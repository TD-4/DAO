...
class _PSPModule(nn.Module):
    def __init__(self, in_channels, bin_sizes, norm_layer):     # in_channels是最后特征图的通道数 2048, bin_sizes:[1,2,3,6]
        super(_PSPModule, self).__init__()
        out_channels = in_channels // len(bin_sizes)    # out_channels=512
        self.stages = nn.ModuleList([self._make_stages(in_channels, out_channels, b_s, norm_layer) 
                                                        for b_s in bin_sizes])      # 创建特征金字塔，ModuleList
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, backbone='resnet152', pretrained=True, use_aux=True, freeze_bn=False, freeze_backbone=False):
        ...
        m_out_sz = model.fc.in_features     # 2048个channels的特征图
