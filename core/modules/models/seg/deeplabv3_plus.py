...
by Remi Cadene
''' 
class SeparableConv2d(nn.Module):   # DSC
...
class Block(nn.Module):
    ...
    def forward(self, x):
        output = self.rep(x)
        if self.skip is not None:   # 是否 有skip
            skip = self.skip(x)
...
class Xception(nn.Module):
    def __init__(self, output_stride=16, in_channels=3, pretrained=True):
        ...
        if output_stride == 16: b3_s, mf_d, ef_d = 2, 1, (1, 2)     # b3_s:block 3 stride 前三个block的stride；mf_d:middle filter dilation; ef_d:end filter dilation
        if output_stride == 8: b3_s, mf_d, ef_d = 1, 2, (2, 4)
        
        # Entry Flow
        self.conv1 = nn.Conv2d(in_channels, 32, 3, 2, padding=1, bias=False)    # down stride
        self.bn1 = nn.BatchNorm2d(32)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(32, 64, 3, 1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(64)

        self.block1 = Block(64, 128, stride=2, dilation=1, use_1st_relu=False)  # down stride
        self.block2 = Block(128, 256, stride=2, dilation=1)     # down stride
        self.block3 = Block(256, 728, stride=b3_s, dilation=1)  # down stride  if output_stride==16: else not down
...
class DeepLab(nn.Module):
    def __init__(self, num_classes, in_channels=3, backbone='xception', pretrained=True, freeze_backbone=False,
