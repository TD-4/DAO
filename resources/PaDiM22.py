...
class PaDiM(KNNExtractor):
    ...
    def fit(self, train_dl):
        ...
        # calcs
        self.means = torch.mean(self.patch_lib, dim=0, keepdim=True)   # torch.Size([200, 1792, 56, 56]) -> torch.Size([1, 1792, 56, 56])
        self.means_reduced = self.means[:, self.r_indices, ...]  # torch.Size([1, 100, 56, 56])
        x_ = self.patch_lib_reduced - self.means_reduced    # torch.Size([200, 100, 56, 56])

        # cov calc
        self.E = torch.einsum(
            'abkl,bckl->ackl',
            x_.permute([1, 0, 2, 3]),  # transpose first two dims, torch.Size([100, 200, 56, 56])
            x_,
        ) * 1 / (self.patch_lib.shape[0] - 1)
        self.E += self.epsilon * torch.eye(self.d_reduced).unsqueeze(-1).unsqueeze(-1)  # torch.Size([100, 100, 56, 56])
        self.E_inv = torch.linalg.inv(self.E.permute([2, 3, 0, 1])).permute([2, 3, 0, 1])   # torch.Size([100, 100, 56, 56])
