...
# @Copy From: https://github.com/rvorias/ind_knn_ad/blob/master/indad/utils.py
...
def get_coreset_idx_randomp(
    ...
    logger.info(f"   Fitting random projections. Start dim = {z_lib.shape}.")
    try:
        transformer = random_projection.SparseRandomProjection(eps=eps)
        z_lib = torch.tensor(transformer.fit_transform(z_lib))
        logger.info(f"   DONE.                 Transformed dim = {z_lib.shape}.")
    except ValueError:
        logger.info("   Error: could not project vectors. Please increase `eps`.")
